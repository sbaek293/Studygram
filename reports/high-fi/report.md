...
#### Sche In
First I guided other members without experience in Unity and Git to get used to the system. My main tasks were the authentication system, multiplayer system where members could see each other moving around, settings menu, the functionality of the card set and group session system. I implemented the backend for these functionalities and also designed the UI for those systems. Afterwards Olesia helped me in polishing the card set and group session system. Additionally I connected the firebase database system over the whole game. I also managed and planned the general overview of our project. Lastly, I did the building and deployment of the application. 
The most difficult part was integrating the Firebase Database into Unity because it was my first time using it. Maintaining data consistency between users, adding new features without breaking existing code was also difficult through the project. Additionally, because Unity was not meant to be used for mainly UI based applications, it was difficult to implement UI design in the game engine. It was my first time actively using AI for programming. So there were difficulties debugging AI generated Code and integrating it into a game engine.
Therefore, I was able to learn a lot about using databases and data saving with JSON in Unity. Additionally, due to the asynchronous way of social interactions it was the first time using event handling or listening and notifying.
